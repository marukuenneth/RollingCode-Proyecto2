...
document.addEventListener('DOMContentLoaded', function () {
    const botonColor = document.getElementById('seleccionarColor-medico'); // ID actualizado
    ...
    const colorGuardado = localStorage.getItem('colorPortadaMedico');
    if (colorGuardado) {
    ...
    botonColor.addEventListener('click', function () {
        ...
        if (colorSeleccionado) {
            ...
            localStorage.setItem('colorPortadaMedico', colorEnMinusculas);
        }
    });
});
...
/* FOTO DE PERFIL */
document.addEventListener('DOMContentLoaded', function () {
    const inputAvatar = document.getElementById('input-avatar-medico'); // ID actualizado
    const avatarImg = document.getElementById('avatar-img-medico'); // ID actualizado

    // Cargar la imagen de perfil desde el almacenamiento local si existe
    const savedAvatar = localStorage.getItem('avatarMedico');
    if (savedAvatar) {
    ...
    inputAvatar.addEventListener('change', function (event) {
        ...
        if (file) {
            ...
            reader.onload = function (e) {
                ...
                localStorage.setItem('avatarMedico', e.target.result);
                avatarImg.style.display = 'block'; // Mostrar la imagen después de cargarla
            };
...
/* DATOS BIOGRAFIA */
document.addEventListener('DOMContentLoaded', function () {
    const bioTextarea = document.getElementById('bio-medico'); // ID actualizado

    // Mostrar la biografía almacenada al cargar la página
    const bioGuardada = localStorage.getItem('bioUsuarioMedico');
    if (bioGuardada) {
    ...
    bioTextarea.addEventListener('input', function () {
        ...
        localStorage.setItem('bioUsuarioMedico', nuevaBio);
    });
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    const btnEditarDatos = document.getElementById('editarDatos-medico'); // ID actualizado
    const btnGuardarCambios = document.getElementById('guardarCambios-medico'); // ID actualizado
    const listaDatosOriginal = document.getElementById('lista-datos-original-medico'); // ID actualizado
    const listaDatosEditable = document.getElementById('lista-datos-editable-medico'); // ID actualizado
    // Objeto que contiene referencias a los campos de entrada de datos
    const inputs = {
        direccion: document.getElementById('input-direccion-medico'), // ID actualizado
        telefono: document.getElementById('input-telefono-medico'), // ID actualizado
        ocupacion: document.getElementById('input-ocupacion-medico'), // ID actualizado
        registro: document.getElementById('input-registro-medico'), // ID actualizado
        nacimiento: document.getElementById('input-nacimiento-medico'), // ID actualizado
        obraSocial: document.getElementById('input-obraSocial-medico'), // ID actualizado
    };
    ...
    btnEditarDatos.addEventListener('click', function () {
        ...
        for (const key in inputs) {
            if (inputs.hasOwnProperty(key)) {
                inputs[key].value = document.getElementById(key + '-medico').textContent; // ID actualizado
            }
        }
    });
    ...
    btnGuardarCambios.addEventListener('click', function () {
        ...
        localStorage.setItem('datosUsuarioMedico', JSON.stringify(datosActualizados));
        ...
        for (const key in datosActualizados) {
            if (datosActualizados.hasOwnProperty(key)) {
                document.getElementById(key + '-medico').textContent = datosActualizados[key]; // ID actualizado
            }
        }
    ...
    const datosGuardados = JSON.parse(localStorage.getItem('datosUsuarioMedico'));
    if (datosGuardados) {
        for (const key in datosGuardados) {
            if (datosGuardados.hasOwnProperty(key)) {
                document.getElementById(key + '-medico').textContent = datosGuardados[key]; // ID actualizado
            }
        }
    }
});
...
document.addEventListener('DOMContentLoaded', function () {
    const resumenTurno = document.getElementById('resumen-turno-medico'); // ID actualizado
    ...
    const formDataString = localStorage.getItem('formularioDataMedico');
    if (formDataString) {
        const formData = JSON.parse(formDataString);

        // Mostrar la fecha y hora de solicitud del turno
        document.getElementById('solicitud-turno-resumen-medico').textContent = solicitudActual; // ID actualizado
        ...
        document.getElementById('fecha-turno-resumen-medico').textContent = fechaHoraTurno; // ID actualizado

        // Mostrar la especialidad del turno
        document.getElementById('especialidad-turno-resumen-medico').textContent = formData['especialidad']; // ID actualizado
    }
});
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    if (formDataString) {
        ...
        const formDataBody = document.getElementById('turnos-medico'); // ID actualizado
    ...
    // Obtener el modal
    const modal = document.getElementById("detalleTurno-medico"); // ID actualizado

    // Obtener el botón que abre el modal
    const btnAbrirModal = document.getElementById("verDetalle-medico"); // ID actualizado
    ...
    const btnEliminarDatos = document.getElementById('eliminarDatos-medico'); // ID actualizado
...
function mostrarContenedorTurnos() {
    ...
    const mensajeNoTurno = document.getElementById('mensaje-no-turno-medico'); // ID actualizado
...
function eliminarDatosFormulario() {
    ...
    localStorage.removeItem('formularioDataMedico');
